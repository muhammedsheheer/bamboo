...
const Reviews = ({}) => {
    ...
    return (
        <section className="relative flex h-full w-full justify-center bg-transparent">
            <div className="flex h-full w-full max-w-[1300px] flex-col items-start justify-center gap-9 py-12 md:py-44">
                <div className="flex h-fit w-full flex-col items-center justify-center gap-2 lg:gap-2">
                    <h1 className="w-full text-center font-playfair text-5xl font-[400] capitalize text-[#DDD1BD]">Signature Flavours</h1>
                </div>
                        ...
                        <Carousel className="w-full px-4">
                            <CarouselContent className="ml-4 flex h-full w-full justify-center gap-4">
                                {reviews.map((review, index) => (
                                    <CarouselItem
                                        key={index}
                                        className="flex w-full basis-full flex-col gap-6 rounded-2xl border border-[#FFE2A2] bg-[#0b0b0b] px-6 py-8 md:basis-1/3 md:rounded-none"
                                    >
                                        <div className="flex w-full items-center gap-2">
                                            ...
                                            <div className="flex flex-col gap-2">
                                                <p className="text-[#FFE2A2]">{review.author_name}</p>
                                                <span className="text-[#FFE2A2]">{review.relative_time_description}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="">
                                            <p className="line-clamp-5 text-[#FFE2A2]">{review.text}</p>
                                        </div>
                                    </CarouselItem>
                                ))}
                            </CarouselContent>
